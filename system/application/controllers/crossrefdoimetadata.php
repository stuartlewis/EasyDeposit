...
class CrossRefDOIMetadata extends EasyDeposit
{
    ...
    function index()
    {
        ...
        else
        {
            ...
            $_SESSION['crossrefdoi-title'] = $this->input->xss_clean($_POST['title']);
            for ($authorpointer = 1; $authorpointer < $_SESSION['crossrefdoi-authorcount']; $authorpointer++)
            {
                $_SESSION['crossrefdoi-author' . $authorpointer] = $this->input->xss_clean($_POST['author' . $authorpointer]);
            }

            $_SESSION['crossrefdoi-year'] = $this->input->xss_clean($_POST['year']);
            $_SESSION['crossrefdoi-volume'] = $this->input->xss_clean($_POST['volume']);
            $_SESSION['crossrefdoi-issue'] = $this->input->xss_clean($_POST['issue']);

            $types = $this->config->item('easydeposit_metadata_itemtypes');
            $_SESSION['crossrefdoi-type'] = $this->input->xss_clean($types[$_POST['type']]);

            $peer = $this->config->item('easydeposit_metadata_peerreviewstatus');
            $_SESSION['crossrefdoi-peerreviewed'] = $this->input->xss_clean($peer[$_POST['peerreviewed']]);
